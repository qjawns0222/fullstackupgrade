...
class AuthServiceTest {
    ...
    fun `reissue - Successful Token Rotation`() {
        ...
        // Use helper any()
        verify(refreshTokenRepository).save(any(RefreshToken::class.java)) // New token saved
    }
    ...
    fun `reissue - Fail if Token Invalid`() {
        ...
        // When & Then
        val exception =
                assertThrows(RuntimeException::class.java) { authService.reissue(invalidToken) }
    ...
    fun `reissue - Fail if Token Not in Redis (Logged out or Reused)`() {
        ...
        // When & Then
        val exception =
                assertThrows(RuntimeException::class.java) { authService.reissue(oldRefreshToken) }
